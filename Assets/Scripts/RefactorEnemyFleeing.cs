...
public class RefactorEnemyFleeing : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            if (playerStats.health == playerStats.playerStartingHealth) 
            {
                Flee(); 
                Debug.Log("flee"); 
            }
            else if (playerStats.health < playerStats.playerStartingHealth) 
            {
                Chase(); 
                Debug.Log(playerStats.health + "chase"); 
            }
	...
	private void Flee()
	{
		...
		//sight.position = new Vector3(player.transform.position.x, transform.position.y, player.transform.position.z);
        Vector3 direction = new Vector3(transform.position.x - player.transform.position.x,transform.position.y, transform.position.z - player.transform.position.z);
		transform.LookAt(direction);
		transform.position = Vector3.MoveTowards(transform.position, direction, Time.deltaTime * enemyStats.chaseSpeed);
	}
